...
const PRIORITY_STATE_TITLE: Record<PriorityStateSlug, Partial<Record<ServiceVertical, string>>> = {
  "new-york": { web: "Web Development Serving New York Businesses | Klikcy", ecommerce: "E-commerce Development Serving New York | Klikcy" },
  "new-jersey": { web: "Web Development Serving New Jersey Businesses | Klikcy" },
  connecticut: { app: "App Development Serving Connecticut Businesses | Klikcy" },
  pennsylvania: { app: "Custom Software Serving Pennsylvania Businesses | Klikcy" },
};
...
function stateTitle(service: Service, state: State): string {
  ...
  const templates: Record<string, ((s: Service, st: State) => string)[]> = {
    ...
    ai: [
      (s, st) => `${s.name} Serving ${st.name} Businesses | Klikcy`,
      (s, st) => `AI Automation Serving ${st.name} Businesses | Klikcy`,
      (s, st) => `${s.name} in ${st.name} | Klikcy`,
    ],
    ecommerce: [
      (s, st) => `${s.name} in ${st.name} | Klikcy`,
...
const PRIORITY_CITY_TITLE: Record<string, Partial<Record<ServiceVertical, string>>> = {
  manhattan: { web: "Web Development Serving Manhattan Businesses | Klikcy" },
  brooklyn: { app: "App Development Serving Brooklyn Businesses | Klikcy" },
  queens: { app: "Custom Software Serving Queens Businesses | Klikcy" },
  "the-bronx": { ai: "AI Automation Serving The Bronx Businesses | Klikcy" },
  "staten-island": { web: "Website Development Serving Staten Island | Klikcy" },
  "jersey-city": { web: "Web Development Serving Jersey City Businesses | Klikcy" },
  stamford: { app: "App Development Serving Stamford Businesses | Klikcy" },
  philadelphia: { app: "Custom Software Serving Philadelphia Businesses | Klikcy" },
  "king-of-prussia": { ai: "AI Automation Serving King of Prussia | Klikcy" },
};
...
function cityTitle(service: Service, city: CityRef): string {
  ...
  const templates: ((s: Service, c: CityRef) => string)[] = [
    ...
    (s, c) => `${s.name} for ${c.name}, ${c.state.abbr} | Klikcy`,
...
export function buildCategoryPageMetadata(category: Category): PageMetadata {
  ...
  const categoryTitles: Partial<Record<Category["slug"], string>> = {
    "web-development": "Web Development Services | Custom Websites & Apps | Klikcy",
    "seo-aeo": "SEO & AEO Services | Rank, Answer & Convert | Klikcy",
    "ai-automation": "AI Automation Services | Smarter Business Systems | Klikcy",
    ecommerce: "E-Commerce Development | Shopify & WooCommerce | Klikcy",
    "branding-design": "Branding & UI/UX Design Services | Klikcy",
    "marketing-growth": "Digital Marketing Strategy Services | Klikcy",
  };
