        ...
        <section className="bg-gradient-hero">
          <div className="container-x py-14 sm:py-20">
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              <Link to="/contact" className="btn-primary">Get a {city.name} quote <ArrowRight className="h-4 w-4" /></Link>
              <Link to={`/${service.slug}/${city.state.slug}`} className="btn-secondary">{service.name} in {city.state.name}</Link>
            </div>
          </div>
        </section>
          ...
          <section className="section bg-[hsl(var(--soft-bg))]">
            <div className="container-x">
              ...
              <div className="mt-6 flex flex-wrap gap-2">
                {otherCities.map((c) => (
                  <Link key={c.slug} to={`/${service.slug}/${city.state.slug}/${c.slug}`} className="rounded-full border border-border bg-white px-4 py-2 text-sm hover:border-primary hover:text-primary">
