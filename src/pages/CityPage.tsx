      ...
      <main>
        <section className="bg-gradient-hero">
          <div className="container-x py-14 sm:py-20">
            ...
            <Link to="/contact" className="btn-primary mt-7">Work with Klikcy in {city.name} <ArrowRight className="h-4 w-4" /></Link>
          </div>
        </section>
        ...
        <section className="section">
          <div className="container-x">
            ...
            <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {featuredSvcs.map((s) => (
                <Link key={s.slug} to={`/${s.slug}/${city.state.slug}/${city.slug}`} className="card-soft group block">
          ...
          <section className="section bg-[hsl(var(--soft-bg))]">
            <div className="container-x">
              ...
              <div className="mt-6 flex flex-wrap gap-2">
                {otherCities.map((c) => (
                  <Link key={c.slug} to={`/service-areas/${city.state.slug}/${c.slug}`} className="rounded-full border border-border bg-white px-4 py-2 text-sm hover:border-primary hover:text-primary">{c.name}</Link>
                ))}
              </div>
              <Link to={`/service-areas/${city.state.slug}`} className="mt-6 inline-flex text-sm font-semibold text-primary">All of {city.state.name} →</Link>
            </div>
          </section>
        )}
      </main>
